...
int main(int argc, char* argv[])
{
    ...
    outfile.open((me.outfilename + ".chkbox.dat.gz").c_str(), std::ios_base::out);
    ...
    std::thread outfileThread = std::thread([&me, &outfile, &trajs, &trajs_buffer_size, &done_calculating, &done_reading, &done_writting](){
        ...
        for (int i_trajs_buffer = 0; i_trajs_buffer < trajs_buffer_size; i_trajs_buffer++)
        {
            ...
            {
                //std::cout << "@@@@" << i_trajs_buffer <<  std::endl;
                if (done_reading && done_calculating)
                {
                    done_writting = true; 
                    //std::cout << "write thread exiting" << done_reading << " " << done_calculating << " " << done_writting << std::endl;
                    return;
                }
                std::this_thread::sleep_for(dura);
            }

            //std::cout << trajs[i].activeFrame_count << std::endl;

            for (int i_frames = 0; i_frames < trajs[i_trajs_buffer].activeFrame_counter; i_frames++)
            {
                PeriodicityCheck::WriteResults(&trajs[i_trajs_buffer].activeFrames[i_frames], &me, outfile);
                //FrameGeometry::WriteOutFrame(&trajs[i_trajs_buffer].activeFrames[i_frames], outfile, &me);
            }
        ...
        for (int i_trajs_buffer = 0; i_trajs_buffer < trajs_buffer_size; i_trajs_buffer++)
        {
            ...
            while (!(trajs[i_trajs_buffer].state=="read"))
            {
                ...
                if (done_reading)
                {
                    ...
                    //std::cout << "calculation thread exiting: " << done_reading << " " << done_calculating << std::endl;
                    return;
                }
                std::this_thread::sleep_for(dura);
            }

            //std::cout << trajs[i_trajs_buffer].activeFrame_count << std::endl;
            ...
            try
            {
                ...
                //std::cout << "$$$$" << trajs[i_trajs_buffer].activeFrame_counter << " " << trajs[i_trajs_buffer].activeFrame_count << " " << perThread << std::endl;

                for (int i_threads = 0; i_threads < me.num_thread_real; i_threads++)
                {
                    //std::cout << "###"<<i_threads<<"..." << i_threads * perThread << " " << ((i_threads + 1) * perThread) << " " << std::endl;
                    myThreads[i_threads] = std::thread([i_threads, &me, &grids, &i_trajs_buffer, &trajs, &perThread, &dura]()
                    {
                        ...
                        {
                            //std::cout << "###"<< i_threads <<"$$$  " << i_frames << " " << trajs[i_trajs_buffer].activeFrames[i_frames].time << std::endl; 
                ...
                {
                    //std::cout << "###***  " << i_remainders << " " << trajs[i_trajs_buffer].activeFrames[i_remainders].time << std::endl; 
