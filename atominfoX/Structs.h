...
public:
	...
	struct GenericParameters {
		...
		//number of threads available supported by processor
		int num_thread_real;
		...
		//e.g. determines what checkbox will write out
		int verbosity;
		...
		//first atom set A, last atom set A, first atom set B, last atom set B, number of elements in the matrix
		Eigen::Matrix<int, 5, Eigen::Dynamic> distanceMatrixSets;
		std::vector<double> distanceMatrixCutOff;
		std::vector<bool> distanceMatrixWriteRaw;
	...
	struct FrameGeometric {
		...
		//checkboxX
		//use to keep tracck of all interactions between atoms in a frame
		std::vector<std::vector<double>> periodic_interactions;

		//distanceMatrixX
		std::vector<Eigen::Matrix<double, 1, Eigen::Dynamic>> interatomicDistances;
	};
